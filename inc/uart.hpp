...
private:
	void pinsInit_()
	{
		constexpr uint32_t RCC_AHB1_PERIPH =
				useUartName == uartName::uart1 ? RCC_AHB1Periph_GPIOA	:
				useUartName == uartName::uart2 ? RCC_AHB1Periph_GPIOB	: RCC_AHB1Periph_GPIOC;

		RCC_AHB1PeriphClockCmd(RCC_AHB1_PERIPH, ENABLE);

		constexpr uint16_t USE_UART_PINS =
				useUartName == uartName::uart1 ? (GPIO_Pin_2 | GPIO_Pin_3)		:
				useUartName == uartName::uart2 ? (GPIO_Pin_10 | GPIO_Pin_11)	: (GPIO_Pin_10 | GPIO_Pin_11);
		...
		GPIO_TypeDef* const useGpioPort =
				useUartName == uartName::uart1 ? GPIOA :
				useUartName == uartName::uart2 ? GPIOB : GPIOC;

		GPIO_Init_reference_(useGpioPort, GPIO_INIT_STRUCT);

		constexpr uint8_t GPIO_AF  =
				useUartName == uartName::uart1 ? GPIO_AF_USART2	:
				useUartName == uartName::uart2 ? GPIO_AF_USART3	: GPIO_AF_UART4;

		constexpr std::pair<uint16_t, uint16_t> GPIO_PIN_SOURCE =
				useUartName == uartName::uart1 ? std::make_pair(GPIO_PinSource2, GPIO_PinSource3)	:
				useUartName == uartName::uart2 ? std::make_pair(GPIO_PinSource10, GPIO_PinSource11)	: std::make_pair(GPIO_PinSource10, GPIO_PinSource11);
	...
	void uartInit_()
	{
		constexpr uint32_t RCC_APB1PERIPH =
				useUartName == uartName::uart1 ? RCC_APB1Periph_USART2	:
				useUartName == uartName::uart2 ? RCC_APB1Periph_USART3	: RCC_APB1Periph_UART4;
	...
	{
		return	useUartName == uartName::uart1 ? USART2 :
...
public:
	...
	UART_Interrupt(uartName use_uart_name, const std::function<void(uint8_t)>&& addFunc) : use_uart_name_(use_uart_name)
	{
		...
		case uartName::uart3:
			uart3_call_functions_.insert(std::make_pair(this, addFunc));
			break;
		}
	}
	...
	static void uart3Callback()
	{
		const uint8_t receive_byte = static_cast<uint8_t>(USART_ReceiveData(UART4));
		for(auto i : uart3_call_functions_)i.second(receive_byte);
	...
	virtual ~UART_Interrupt()
	{
		...
		case uartName::uart3:
			uart3_call_functions_.erase(this);
			break;
		}
	}
private:
	...
	static std::map<UART_Interrupt* const,const std::function<void(uint8_t)>> uart3_call_functions_;
};
